...
const TOKUSHOHO_JA = `# 特定商取引法に基づく表記
...
【代表者名】
個人開発者（請求に応じて開示）
※ 開示をご希望の方はサポートメールよりご連絡ください。

【所在地】
請求に応じて遅滞なく開示いたします。
※ 個人事業主のため住所は請求があった際に開示します。

【電話番号】
請求に応じて遅滞なく開示いたします。
※ お問い合わせはメールにて優先的に対応しております。

【メールアドレス（サポート窓口）】
yosihiro1988ty@yahoo.co.jp
...
【支払方法】
Apple Inc.が提供するApp Store（Apple In-App Purchase）を通じたお支払いのみ対応しています。ご利用可能な支払手段は、お客様のApple IDに登録されたお支払い方法（クレジットカード、デビットカード、Apple Payなど）に準じます。

【支払時期】
購入手続き完了時にApple Inc.を通じて即時決済されます。サブスクリプション型サービスの場合、購入日を起算日として自動的に更新され、更新日に決済されます。
...
返金をご希望の場合は、Apple Inc.のサポートページ
https://support.apple.com/ja-jp/billing
よりお手続きください。返金の可否はAppleの判断に委ねられます。
...
【サービスの動作環境】
・iOS 16.0以上を搭載したiPhone、iPad
・インターネット接続が必要です

【サービスの有効期間】
・都度購入型: 購入後、サービス提供期間中は継続してご利用いただけます
・サブスクリプション型: 各購入日を起算日として設定された期間（月次または年次）ご利用いただけます。自動更新をオフにしない限り、期間終了後に自動的に更新されます
...
【その他の特記事項】
・本サービスのアプリ内課金はApple Inc.のIn-App Purchase（IAP）を通じて処理されます。当社はお客様の決済情報を直接取得・保持しません。
・本サービスはインターネット接続が必要なオンラインサービスです。通信障害等による一時的なサービス不能について、当社は責任を負いかねる場合があります。

本表記は 2026年06月24日 時点の内容です。`;
